...
abstract class SwipeMenuHelper(
    context: Context,
    ...
    private var buttonList: MutableList<MenuButton>? = null
    lateinit var gestureDetector: GestureDetector
    var swipePos = -1
    var swipeThreshold = 0.5f
    val buttonBuffer: MutableMap<Int, MutableList<MenuButton>>
    lateinit var removerQueue: LinkedList<Int>

    abstract fun instantiateMenuButton(
    ...
    override fun onChildDraw(
        ...
        if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE) {
            if (dX < 0) {
                var buffer: MutableList<MenuButton> = ArrayList()
                if (!buttonBuffer.containsKey(pos)) {
                    instantiateMenuButton(viewHolder, buffer)
